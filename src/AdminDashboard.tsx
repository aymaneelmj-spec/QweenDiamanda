...
:root{
  ...
  --red:#C94040;--green:#40A870;--amber:#C9A040;
  --blue:#4070C9;
}
.light{
  ...
  --red:#A03030;--green:#2A7850;--amber:#907030;
  --blue:#2050A0;
}
body{font-family:'DM Sans',sans-serif;background:var(--bg);color:var(--text);min-height:600px;font-size:14px;line-height:1.5;overflow-x:hidden}

/* SCREENS */
.screen{display:none;width:100%;min-height:600px}
.screen.active{display:flex}

/* ── LOGIN ── */
#screen-login{align-items:center;justify-content:center;flex-direction:column;min-height:600px;position:relative;overflow:hidden}
...
.desktop-block{display:none;text-align:center;padding:40px 20px;color:var(--text2)}
...
/* STRENGTH METER */
.strength-bar{height:4px;border-radius:2px;transition:all .3s;flex:1}
...
/* SEPARATOR */
.gold-line{height:1px;background:linear-gradient(90deg,transparent,var(--gold),transparent);margin:20px 0;opacity:.3}
...
<!-- MOBILE BLOCK -->
<div id="mobile-block" style="display:none;text-align:center;padding:60px 24px;min-height:600px;align-items:center;flex-direction:column;justify-content:center">
  <div style="font-size:48px;margin-bottom:16px">🖥️</div>
  <h2 style="font-family:'Cormorant Garamond',serif;color:var(--gold);font-size:22px;margin-bottom:8px">Desktop Only</h2>
  <p style="color:var(--text2);font-size:14px">The Queen Diamanda Admin Dashboard<br>is designed for desktop use only.</p>
</div>
...
<div class="screen active" id="screen-login">
  ...
  <div class="login-card">
    ...
    <div class="form-group">
      ...
      <div class="pw-wrap">
        <input type="password" id="login-pw" placeholder="••••••••••••" autocomplete="current-password" oninput="checkPwStrength(this.value)" onkeydown="checkCaps(event)" onkeyup="checkCaps(event)">
  ...
  <div style="display:flex;flex-direction:column;width:100%;min-height:600px">
    ...
    <div class="topbar">
      ...
      <div class="topbar-nav">
        ...
        <button class="nav-btn" onclick="showTab('orders',this)">WhatsApp Orders</button>
    ...
    <div class="main-content" style="flex:1">
      ...
      <div class="tab active" id="tab-overview">
        ...
        <div class="stat-grid">
          ...
          <div class="stat-card">
            <div class="stat-label">WhatsApp Orders</div>
          ...
          <div class="stat-card">
            ...
            <div class="stat-sub">DH this month</div>
          </div>
          <div class="stat-card">
            <div class="stat-label">Media Files</div>
            <div class="stat-value" id="stat-media">6</div>
        ...
        <div class="quick-grid">
          <div class="panel">
            ...
            <div id="activity-feed">
              ...
              <div class="activity-item"><div class="activity-dot blue"></div><div><div>Product "Set Bracelets Rouge" edited</div><div class="activity-time">Yesterday 14:30</div></div></div>
              ...
              <div class="activity-item"><div class="activity-dot red"></div><div><div>Login attempt failed (wrong pw)</div><div class="activity-time">3 days ago</div></div></div>
            </div>
          </div>
          <div class="panel">
            <h3>Quick Actions</h3>
            <div style="display:flex;flex-direction:column;gap:8px">
              <button class="btn btn-secondary" style="text-align:left;justify-content:flex-start;display:flex;align-items:center;gap:8px" onclick="openAddProduct()">📦 Add New Product</button>
              <button class="btn btn-secondary" style="text-align:left;justify-content:flex-start;display:flex;align-items:center;gap:8px" onclick="showTab('media',document.querySelectorAll('.nav-btn')[2])">🖼 Upload Media</button>
              <button class="btn btn-secondary" style="text-align:left;justify-content:flex-start;display:flex;align-items:center;gap:8px" onclick="showTab('orders',document.querySelectorAll('.nav-btn')[3])">💬 View Orders</button>
              <button class="btn btn-secondary" style="text-align:left;justify-content:flex-start;display:flex;align-items:center;gap:8px" onclick="showTab('settings',document.querySelectorAll('.nav-btn')[4])">⚙️ Store Settings</button>
              <button class="btn btn-secondary" style="text-align:left;justify-content:flex-start;display:flex;align-items:center;gap:8px" onclick="showTab('security',document.querySelectorAll('.nav-btn')[5])">🔐 Security Log</button>
            </div>
          </div>
        </div>
      ...
      <div class="tab" id="tab-products">
        ...
        <div class="products-toolbar">
          <input class="search-input" type="text" placeholder="Search products..." id="search-input" oninput="filterProducts()">
          <select class="filter-select" id="filter-badge" onchange="filterProducts()">
        ...
        <div class="products-list" id="products-list">
          <table>
            <thead><tr><th></th><th>Image</th><th>Name</th><th>Price</th><th>Badge</th><th>Status</th><th>Actions</th></tr></thead>
      ...
      <div class="tab" id="tab-settings">
        <div style="margin-bottom:20px">
          <h2 style="font-family:'Cormorant Garamond',serif;font-size:22px;font-weight:600">Store Settings</h2>
        </div>

        <div class="settings-section">
          <h3>Store Information</h3>
          <div class="setting-row">
            <div><label>Store Name</label><p>Display name across the website</p></div>
            <input class="setting-input" value="Queen Diamanda" id="set-name">
          </div>
          <div class="setting-row">
            <div><label>WhatsApp Number</label><p>For order notifications</p></div>
            <input class="setting-input" value="212784854164" id="set-wa">
          </div>
          <div class="setting-row">
            <div><label>Currency</label><p>Displayed on product prices</p></div>
            <input class="setting-input" value="DH" id="set-currency">
          </div>
        ...
        <div class="settings-section">
          ...
          <div class="mform-group">
            ...
            <input type="password" class="mform-group input" style="width:100%;background:var(--bg3);border:1px solid var(--border);border-radius:8px;padding:9px 12px;color:var(--text);font-family:'DM Sans',sans-serif;font-size:13px;outline:none" id="cur-pw">
          </div>
          <div class="mform-group">
            ...
            <div class="pw-strength" style="margin-top:6px" id="pw-strength2">
              <span id="s21"></span><span id="s22"></span><span id="s23"></span><span id="s24"></span>
            </div>
          </div>
          <div class="mform-group">
            <label>Confirm New Password</label>
            <input type="password" style="width:100%;background:var(--bg3);border:1px solid var(--border);border-radius:8px;padding:9px 12px;color:var(--text);font-family:'DM Sans',sans-serif;font-size:13px;outline:none" id="conf-pw">
          </div>
          <div style="text-align:right;margin-top:12px">
...
<div class="modal-overlay" id="modal-order">
  <div class="modal" style="max-width:420px">
    ...
    <div class="mform-group"><label>Status</label>
      <select id="o-status"><option value="new">New</option><option value="pending">Pending</option><option value="done">Delivered</option></select>
    </div>
    <div class="modal-actions">
...
<script>
// ── STATE ──────────────────────────────────────────────
const CORRECT_EMAIL = 'admin@queendiamanda.ma';
let CORRECT_PW = 'QueenDiamanda@2025!';
let attempts = 0;
let locked = false;
let lockTimer = null;
let sessionInterval = null;
let sessionLeft = 30 * 60;
...
let products = [
  ...
  {id:2,name:'Bracelet Trèfle Arc-en-Ciel',price:89,image:'/images/2.png',badge:'Nouveau',hidden:false},
  {id:3,name:'Bracelet Vague de Cristaux',price:75,image:'/images/3.png',badge:null,hidden:false},
  {id:4,name:'Bracelet Corde H Coloré',price:65,image:'/images/4.png',badge:null,hidden:false},
];
...
let orders = [
  ...
  {id:2,name:'Nadia El Fassi',city:'Rabat',product:'Bracelet Trèfle Arc-en-Ciel',status:'pending',date:'Jun 6 2026'},
  {id:3,name:'Fatima Zahra',city:'Marrakech',product:'Bracelet Vague de Cristaux',status:'new',date:'Jun 7 2026'},
];

let securityLog = [
  {type:'login',icon:'✅',action:'Successful login from Admin',time:'Today 09:00'},
  {type:'edit',icon:'✏️',action:'Product "Set Bracelets" price updated → 149 DH',time:'Yesterday 14:30'},
  {type:'fail',icon:'❌',action:'Failed login attempt: wrong password',time:'Jun 5 13:12'},
  {type:'delete',icon:'🗑',action:'Media file removed',time:'Jun 4 10:44'},
];
...
const colors=['var(--red)','var(--amber)','#8BC34A','var(--green)'];
function checkPwStrength(pw){
  const s=getStrength(pw);
  for(let i=1;i<=4;i++){
    const el=document.getElementById('s'+i);
    if(el) el.style.background=i<=s?colors[s-1]:'var(--border)';
  }
}
...
function attemptLogin(){
  ...
  if(email===CORRECT_EMAIL && pw===CORRECT_PW){
    attempts=0;
    addLog('login','✅','Admin logged in successfully','Just now');
    enterDash();
  } else {
    attempts++;
    addLog('fail','❌','Failed login attempt ('+attempts+'/5)','Just now');
    if(attempts>=5){
      locked=true;
      document.getElementById('btn-login').disabled=true;
      let countdown=30;
      const lo=document.getElementById('lockout-msg');
...
function startSession(){
  ...
  sessionInterval=setInterval(()=>{
    ...
    if(disp){disp.textContent=str;disp.className='session-timer'+(sessionLeft<300?' warn':'');}
...
function logout(){
  ...
  addLog('login','🔓','Session ended / Admin signed out','Just now');
...
function filterProducts(){
  ...
  return products.filter(p=>{
    ...
    const badgeMatch=!badge||(badge==='none'?!p.badge:p.badge===badge);
    const statusMatch=!status||(status==='hidden'?p.hidden:!p.hidden);
    return nameMatch&&badgeMatch&&statusMatch;
  });
}
...
function renderProducts(){
  ...
  list.forEach(p=>{
    ...
    card.innerHTML=`
      <div class="img-area">
        ...
        ${p.badge?`<span class="badge ${p.badge==='Bestseller'?'badge-bestseller':'badge-new'}">${p.badge}</span>`:''}
        ${p.hidden?'<span class="badge badge-hidden" style="top:30px">Hidden</span>':''}
        <div class="select-check ${sel?'checked':''}" onclick="toggleSelect(${p.id},event)">${sel?'✓':''}</div>
      </div>
      <div class="card-body">
        <div class="card-title">${p.name}</div>
        <div class="card-price">${p.price} DH</div>
        <div class="card-actions">
          <button class="btn btn-sm btn-secondary" onclick="openEditProduct(${p.id})">Edit</button>
          <button class="btn btn-sm btn-secondary" onclick="toggleHide(${p.id})">${p.hidden?'Show':'Hide'}</button>
          <button class="btn btn-sm btn-danger" onclick="confirmDelete(${p.id})">Del</button>
        </div>
      </div>`;
    ...
    tr.innerHTML=`
      <td><input type="checkbox" ${sel?'checked':''} onchange="toggleSelect(${p.id},event)" style="cursor:pointer"></td>
      <td><div style="width:40px;height:40px;background:var(--bg4);border-radius:6px;display:flex;align-items:center;justify-content:center">💎</div></td>
      <td style="font-weight:500">${p.name}</td>
      <td style="font-family:'Cormorant Garamond',serif;font-size:16px;color:var(--gold)">${p.price} DH</td>
      <td>${p.badge?`<span class="badge ${p.badge==='Bestseller'?'badge-bestseller':'badge-new'}">${p.badge}</span>`:'-'}</td>
      <td><span style="font-size:11px;padding:2px 8px;border-radius:10px;${p.hidden?'background:rgba(201,64,64,.12);color:var(--red)':'background:rgba(64,168,112,.12);color:var(--green)'}">${p.hidden?'Hidden':'Visible'}</span></td>
  ...
  bulk.classList.toggle('show',selectedIds.size>0);
...
function setView(v){
  ...
  gb.classList.toggle('active',v==='grid');
...
function toggleHide(id){
  ...
  if(!p) return;
  p.hidden=!p.hidden;
  addLog(p.hidden?'edit':'edit',p.hidden?'👁':'👁','Product "'+p.name+'" '+(p.hidden?'hidden':'shown'),'Just now');
...
function confirmDelete(id){
  ...
  document.getElementById('confirm-btn').onclick=()=>deleteProduct(id);
  document.getElementById('modal-confirm').classList.add('open');
}

function deleteProduct(id){
  const p=products.find(x=>x.id===id);
  addLog('delete','🗑','Product "'+p.name+'" deleted','Just now');
...
function bulkHide(){
  selectedIds.forEach(id=>{const p=products.find(x=>x.id===id);if(p){p.hidden=true;}});
  addLog('edit','👁','Bulk hide: '+selectedIds.size+' products','Just now');
...
function handleImageUpload(input){
  ...
  document.getElementById('upload-preview').textContent='✅ '+file.name+' ready';
...
function saveProduct(){
  ...
  const image=document.getElementById('p-image').value||'/images/product.png';
  if(!name||!price){showToast('Please fill name and price','error');return;}
  if(currentEditId){
    const p=products.find(x=>x.id===currentEditId);
    if(p){Object.assign(p,{name,price,badge,image});addLog('edit','✏️','Product "'+name+'" updated','Just now');}
    showToast('Product updated','info');
  } else {
    products.push({id:Date.now(),name,price,badge,image,hidden:false});
    addLog('login','📦','New product "'+name+'" added','Just now');
...
function renderMedia(){
  ...
  mediaFiles.forEach(f=>{
    ...
    el.innerHTML='<div class="media-icon">'+f.icon+'</div><div class="media-name">'+f.name+'</div>';
    grid.appendChild(el);
  });
...
function addMedia(input){
  ...
  addLog('edit','🖼','Media uploaded: '+file.name,'Just now');
  renderMedia();
}
...
function renderOrders(){
  ...
  orders.forEach(o=>{
    ...
    div.innerHTML=`
      ...
      <div style="flex:1"><div style="font-weight:500">${o.name}</div><div style="font-size:11px;color:var(--text3)">${o.city}</div></div>
      <div style="flex:1;font-size:13px;color:var(--text2)">${o.product}</div>
      <div style="flex:0 0 90px"><span class="order-status status-${o.status}">${o.status==='new'?'🆕 New':o.status==='pending'?'⏳ Pending':'✅ Delivered'}</span></div>
      <div style="font-size:11px;color:var(--text3)">${o.date}</div>
      <select onchange="updateOrderStatus(${o.id},this.value)" style="background:var(--bg3);border:1px solid var(--border);border-radius:6px;padding:4px 8px;color:var(--text);font-size:12px;cursor:pointer">
        <option ${o.status==='new'?'selected':''} value="new">New</option>
        <option ${o.status==='pending'?'selected':''} value="pending">Pending</option>
        <option ${o.status==='done'?'selected':''} value="done">Delivered</option>
      </select>`;
...
function openAddOrder(){
  ...
  if(sel){sel.innerHTML='';products.forEach(p=>{const op=document.createElement('option');op.value=p.name;op.textContent=p.name;sel.appendChild(op);});}
...
function saveOrder(){
  ...
  orders.unshift({id:Date.now(),name,city,product,status,date:new Date().toLocaleDateString('en-GB',{month:'short',day:'numeric',year:'numeric'})});
  addLog('edit','💬','New order logged for '+name,'Just now');
...
function updateOrderStatus(id,status){
  ...
  if(o){o.status=status;addLog('edit','💬','Order status updated → '+status,'Just now');}
  renderOrders();
}

// ── SETTINGS ───────────────────────────────────────────
function saveSettings(){
  addLog('edit','⚙️','Store settings updated','Just now');
...
function changePassword(){
  ...
  CORRECT_PW=np;
  addLog('edit','🔑','Password changed successfully','Just now');
...
function addLog(type,icon,action,time){
  securityLog.unshift({type,icon,action,time});
  if(securityLog.length>50) securityLog.pop();
  renderLog();
  addActivity(icon,action,time);
}
...
function renderLog(){
  ...
  if(!list) return;
  list.innerHTML='';
  securityLog.forEach(l=>{
    const row=document.createElement('div');
    row.className='log-row';
    row.innerHTML=`<div class="log-icon">${l.icon}</div><div class="log-action">${l.action}</div><span class="log-type log-${l.type}">${l.type}</span><div class="log-time">${l.time}</div>`;
...
function addActivity(icon,text,time){
  ...
  if(!feed) return;
  const dot={login:'green',edit:'blue',delete:'red',fail:'amber'}[text.includes('delete')||text.includes('Delete')?'delete':text.includes('fail')||text.includes('Failed')?'fail':text.includes('added')||text.includes('logged in')?'login':'edit']||'blue';
...
function renderOverviewProducts(){
  const c=document.getElementById('overview-products');
  if(!c) return;
  c.innerHTML='';
...
function updateStats(){
  ...
  const sm=document.getElementById('stat-media');
  if(sp) sp.textContent=products.filter(p=>!p.hidden).length;
